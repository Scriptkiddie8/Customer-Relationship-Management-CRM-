...
// Get the subtask ID from the URL
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['start_tracking'])) {
    ...
    // Insert new time tracking entry into the database
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['complete_subtask'])) {
    ...
    if ($stmt->execute()) {
        ...
        $stmt_note = $link->prepare("INSERT INTO notes (subtask_id, note_text, file_path) VALUES (?, ?, ?)");
        $stmt_note->bind_param("iss", $subtask_id, $note_text, $file_path);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_comment'])) {
    ...
    // Insert comment and file path into the notes table or relevant table
...
<style>
    ...
    <div class="container">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <!-- <th><button class="btn btn-danger">End Time</button></th>
                ...
                <tbody>
                    <?php while ($tracking = $time_tracking_result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <!-- <td><?php echo $tracking['end_time'] ? htmlspecialchars($tracking['end_time']) : 'N/A'; ?></td>
                            <td><?php echo $tracking['duration'] ? htmlspecialchars($tracking['duration']) : 'N/A'; ?></td>
                            <td>
                                <?php if (!$tracking['end_time']): ?>
                                    <form method="post" action="">
                                        <input type="hidden" name="tracking_id" value="<?php echo $tracking['tracking_id']; ?>">
                                        <button type="submit" name="stop_tracking" class="btn btn-danger">Stop Tracking</button>
                                    </form>
                                <?php endif; ?>
                            </td> -->
                        </tr>
        ...
        <div class="modal fade" id="completeSubtaskModal" tabindex="-1" aria-labelledby="completeSubtaskModalLabel"
            aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <form method="post" action="" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <textarea id="note_text" name="note_text" rows="4" class="form-control"
                                    required></textarea>
                            </div>
                            <div class="form-group mt-2">
                            ...
                            <button type="submit" name="complete_subtask" class="btn btn-success mt-2">Complete
                                Subtask</button>
                        </form>
                    </div>
                </div>
